 ...
 *
 * First let's define a domain. (All the following requires scala 2.9.x and scalaz 7.0.5)
 */
...
object GayBar extends Nightclub {
  ...
  def costToEnter(p : Person) : ValidationNel[String, Double] = {
    ...
    (checks map {(_ : (Person => Validation[String, Person])).apply(p).toValidationNel}).sequenceU map { 
       case c :: _ => c.age + 1.5D
       case Nil => 0D // can't happen, just to make the compile shut up
    }
  }
